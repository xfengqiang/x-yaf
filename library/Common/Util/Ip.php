...
class Ip {
    ...
    */
    ...
    static function ipv4_in_range($ip, $range) {
        if (strpos($range, '/') !== false) {
            ...
            // range might be 255.255.*.* or 1.2.3.0-1.2.3.255 or 1.2.3.0
            if (strpos($range, '*') !==false) { // a.b.*.* format
                // Just convert to A-B format by setting * to 0 for A and 255 for B
                $lower = str_replace('*', '0', $range);
                $upper = str_replace('*', '255', $range);
                $range = "$lower-$upper";
            }else if (strpos($range, '-')!==false) { // A-B format
    ...
    static function ipv6_in_range($ip, $range_ip)
    {
